...
def fit_fxn(filename, poly_odr):
    ...
    poly_coeff   = np.polynomial.polynomial.polyfit(main_data['time delay'], main_data['ratio'], poly_odr)
    ...
    main_data.insert(len(main_data.columns), 'poly res', main_data['ratio'] - main_data['poly fit'] ) 
    ...
    fig_ax = main_data.plot.scatter(x = 'time delay', y = 'ratio',color = 'blue', fontsize = 15, title = filename) #define set of figure axis
